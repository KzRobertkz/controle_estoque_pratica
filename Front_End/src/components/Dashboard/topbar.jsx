...
export const Topbar = ({ isCalendarHidden = false }) => {
  ...
  // Fecha o calendário se ele foi escondido
  useEffect(() => {
    if (isCalendarHidden) {
  ...
  const handleCalendarClick = () => {
    // Só permite abrir/fechar se o calendário não estiver escondido
    if (!isCalendarHidden) {
  ...
  const handleDateChange = (date) => {
    ...
    // não echa o calendário após selecionar uma data
  ...
  return (
    <div className='border-b border-stone-400 px-4 mb-4 mt-2 pb-4'>
      <div className='flex items-center justify-between p-0.5'>
        ...
        {/* Só renderiza o calendário se não estiver escondido */}
        {!isCalendarHidden && (
          <div className='relative'>
            <button 
              onClick={handleCalendarClick}
              className='flex text-sm items-center gap-2 bg-zinc-700 transition-colors hover:bg-zinc-500 text-zinc-100 px-3 py-2 rounded focus:outline-none'
            >
              <FiCalendar />
